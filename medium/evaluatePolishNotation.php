...
class Solution {
    ...
    function evalRPN($tokens) {
        ...
        $operators = ['+', '-', '*', '/'];
        $stack = []; // temporarily store integers
        
        foreach($tokens as $t) {
            if (intval($t)) {
                array_push($stack, $t);
            } elseif (in_array($t, $operators)) {
                $stack = $this->doCalculation($t, $stack);
            }
        }
        return array_pop($stack);
    }
    function doCalculation(string $operator, array &$stack) {
        switch ($operator) {
            case '+':
